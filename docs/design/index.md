...
This section answers why the system is shaped the way it is. It is for somebody changing the engine, forking it, or trying to understand a decision they disagree with. If you only want to run a journal or publish a paper, Start and Guides are enough, and nothing here is required reading.
...
The journal repository holds the journal's name, its branding, the editorial checks a submission has to pass, the editions papers are grouped into, and the registry of what has been published. It holds no manuscripts.

Each paper repository holds one manuscript and knows almost nothing about the journal. It builds itself: its CI checks out the engine, builds the paper into a website and a PDF, and deposits it to Zenodo when a version is published.
...
What the engine adds is everything around a build that a single paper cannot know by itself: where the journal's settings live, which version of the engine to run, what the editorial checks are, how a preview gets deployed, how a DOI gets reserved, and how an author's pull request gets built without handing it a token.

MyST is called as a library rather than as a subprocess. `src/myst.ts` constructs a MyST `Session` and calls `loadConfig` and `build` on it directly ([R51]). It is the only module in the engine that imports myst-cli, so every other module is testable without the toolchain installed.
...
The assembled result is written to **`myst.oak.yml`**, next to the paper's `myst.yml`, and MyST is pointed at that file instead. It opens with a banner saying it is generated, the paper template gitignores it, and it is rewritten on every build.
...
One consequence to know about: `myst.oak.yml` is not cleaned up when the build ends. MyST's HTML build finishes by calling `process.exit(0)`, so anything registered to run at the end of the process never runs on a successful build. Deleting the file by hand is always safe.

## The pages here

Each page takes one part of the system and explains why it is built that way. What each part *does* belongs in Reference; this section is only for the reasoning, and it exists because that reasoning is otherwise invisible in the code.
